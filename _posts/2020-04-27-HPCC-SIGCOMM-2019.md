...
（3） **利用INT计算 $I$**. 注意每个发送方需要独立地估计第$j$段链路上的 $I_{j}$ 。假设：所有流具有相同的base RTT $T$

  $I_{j} = qlen + txRate \times T$

  $txRate = \frac{ack_{1}.txBytes - ack_{0}.txBytes}{ack_{1}.ts - ack_{0}.ts}$

  $I_{j}$ 就是对link $j$ $inflight$的估计。这里体现出INT的必要性：需要获取$txRate$这个关键参数

（4）调速策略

  乘性减窗口：factor: $k_{j} = \frac{I_{j}}{\B_{j} \times T}$
