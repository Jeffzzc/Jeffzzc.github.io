...
无论是PCN还是HPCC，二者因为需要获取更多信息来进行拥塞控制（PCN需要交换机根据PAUSE信息标记数据包，接收端采集接收速率，HPCC直接需要全网部署INT技术收集$txBytes$、$timestamp$等），都需要对fabric进行较大改动。笔者认为，这都体现了无损以太网拥塞控制与有损网络的根本性不同，网络状态的增加导致原先有损网络中仅依赖于队长的控制难以达到很好的效果，无损以太网的拥塞管理到底应该是个什么样的架构还有待研究。
