...
这里放一张论文中的结果图，可以看到其实DCTCP，这种根据队长利用multi-bit信息去做速率调整的协议本身在无损网络中性能并不很差（不考虑kernel bypass），甚至超过DCQCN和Timely很多（小流），论文认为是由于DCTCP的窗口相当于一定程度上限制了inflight bytes。笔者认为，DCQCN与DCTCP相比，DCTCP是根据拥塞程度进行细粒度调整，而DCQCN是根据一个周期内有无拥塞进行调整，这方面也可能造成性能差距。
