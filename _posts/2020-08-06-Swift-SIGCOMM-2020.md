...
- 区分fabric congestion 和 endpoint congestion, 这个思想在之前的数据中心拥塞控制协议中没有强调 （就笔者所知, 只有一篇[终端rate limiter](https://wisr.cs.wisc.edu/papers/swratelimit-he.pdf) 的工作提到endpoint congestion）, 主要是因为仍然认为fabric congestion是拥塞的主要种类, fabric 的 congestion 对性能影响最大；但是随着网络带宽增加, 应用需求增高, endpoint congestion越来越不容忽视; 区分这两类拥塞也能更好地进行异常定位; 
