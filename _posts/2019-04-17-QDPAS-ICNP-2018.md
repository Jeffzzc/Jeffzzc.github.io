...
- Large Flow Rerouting
  ...
  ***如果当队列长度超过$S$就进行reroute***, 最大拥塞窗口$W_{2}$为：
  ...
  (1)队列长度小于$S$,拥塞窗口从1增加到$W_{2}$。需要$W_{2}$个RTT,平均窗口为$\frac{W_{2}}{2}$。 在这一阶段发送的流大小$S_{1}$为：
  ...
  (2)平均速率从$r_{1}$降为$\frac{r_{1}}{2}$，再升至$r_{2}$。平均排队延时为$\frac{S}{2C}$。则$r_{2}$:
